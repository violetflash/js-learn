...
// что мы получим результат аналогичный методу push без применения его. Эмуляция заключается в присвоение значения
// как последнего элемента массива. Как вычислить индекс последнего элемента используя длину массива - вы знаете из предыдущего урока.
...
function f12() {
    ...
    document.querySelector(".out-12").innerHTML = output;
}
...
function f13() {
    ...
    for (let i = d13.length - 1; i >= 0 ; i--) {
        reversed[reversed.length] = d13[i];
    }
    d13 = reversed;
    showArr('.out-13', d13);
}
