...
function f13() {
    let block = document.createElement("div");
    block.innerHTML = "pushMe";
    block.classList.add("bg-13");
    block.onclick = f13_1;
...
function f17() {
    let block = document.createElement("div");
    block.innerHTML = "17";
    block.classList.add("bg-17");
    let out = document.querySelector(".out-17");
    out.replaceWith(block);
}
...
function f19() {
    let inputs = document.querySelectorAll(".p-19");
    let str = "";
    for (let i = 0; i < inputs.length; i++) {
        str += `${inputs[i].getAttribute("data-b")} `
    }
