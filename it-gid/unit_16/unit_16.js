...
function f6() {
    /*
    let counter = 0; //индекс
    let temp = [];
    for ( let a6Value of a6 ) {
        for ( let subValue of a6Value ) {
            temp[counter] = subValue;
            document.querySelector('.out-6').textContent += `${subValue} `;
            counter++;
        }
    }
    a6 = temp;
    */
    ...
    a6 = [].concat(...a6);
    for ( let a6Value of a6 ) document.querySelector(".out-6").textContent += `${a6Value} `;
}
...
// В данном случае это 4. Т.е. самый большой вложенный массив это [0,0,0,0,0],
...
function f11() {
    let str = "";
    for (let a11Key in a11) {
        console.log(a11[a11Key]);
        if ( a11[a11Key] > 10 )  str += `${a11[a11Key]} `;
    }
