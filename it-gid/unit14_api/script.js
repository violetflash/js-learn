...
weather_option.onchange = chooseMethod(weather_option, days);

function chooseMethod(select) {
    if (select.value === "current") {
        method = `weather`;
        if ( document.querySelector("label") ) {
            document.querySelector("label").remove();
        }
    } else if (select.value === "forecast" && !document.querySelector(".weather__days")) {
        method = `forecast`;
        select.after(days);
    }
}
